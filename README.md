...
5. The third part is an optional [destination section](https://github.com/deadtype/newdistrict/blob/master/_includes/modal_addtocart/modal_destination_section.html) which only appears when the product being selected is available for both regular order (unconstrained ordering and case based) and 'mix and match'. It provides user with information about the options of adding the product and buttons to select the options.
